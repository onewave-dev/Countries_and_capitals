...
- После каждого ответа живой игрок получает сообщение «✅ Верно» или «❌ Неверно. Правильный ответ: …» без инлайн‑клавиатуры. При верном ответе добавляется название страны, строка «Столица: {capital}» (если вопрос про столицу) и изображение флага.
...
Случайные факты о странах и столицах сохраняются в кэше в формате **JSON**. Путь к файлу задаётся переменной окружения `FACTS_CACHE_PATH`.

- **Первичная генерация** выполняется скриптом:
  ```bash
  python scripts/generate_facts.py
  ```
- **Регулярное обновление** выполняет скрипт:
  ```bash
  python scripts/refresh_facts.py
  ```
  Его запускает cron‑задача не реже одного раза в **5 дней**, проверяя возраст кэша и перезаписывая файл при необходимости.

**Пример Render Cron Job** (обновление каждые 5 дней):
```yaml
schedule: "0 0 */5 * *"
command: "python scripts/refresh_facts.py"
```
Обновление выполняется в фоне и не блокирует работу бота.
