...
- Бот-соперник никогда не получает уже решённый вопрос: если второй игрок отвечает верно, боту задаётся новая пара.
- После каждого вопроса бот рассылает итог обоим игрокам, выполняя роль «виртуального общего чата».
- При каждом верном ответе (игрока, бота или дамми-игрока) всем участникам отправляется сообщение с изображением флага и подписью «<имя> отвечает верно! Правильный ответ: <страна> — <столица>».
- Полный текст каждого вопроса рассылается всем участникам независимо от очерёдности: активный игрок получает сообщение с инлайн-клавиатурой ответов, остальные — тот же текст без клавиатуры.
