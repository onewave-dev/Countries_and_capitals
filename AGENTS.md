...
Для администратора в меню доступен отдельный пункт «[адм.] Тестовая игра» для быстрой проверки кооперативного режима: после выбора континента матч сразу запускается с единой встроенной точностью ботов (~90 % правильных ответов), вторым напарником выступает встроенный бот-помощник, поэтому этап приглашения второго игрока не показывается.

### 1) «Флэш-карточки» (обучение, 1 игрок)
Перед стартом пользователь выбирает, учить ли «все» столицы выбранного континента или запустить режим «Учить по подкатегориям». Во втором случае доступны опции «Совпадает со страной» (берутся пары, где столица записана так же, как название государства), «Столица на выбранную букву» (бот просит ввести одну букву и оставляет только столицы, начинающиеся с неё) и «Остальные столицы» (все оставшиеся страны континента после исключения двух первых групп).

До показа карточек бот выводит список стран и столиц выбранной выборки в формате режима «Учить по спискам» и добавляет под ним кнопки «Перейти к карточкам», «Назад» и «В меню».
...
### 3) «Кооператив: столицы мира» (2 игрока против двух ботов)

- Инициация матча не изменилась: первый игрок создаёт сессию, вводит имя и приглашает напарника. После ввода имён оба участника по очереди выбирают континент (Европа / Азия / Африка / Северная Америка / Южная Америка / Австралия и Океания / Весь мир — 30 случайных пар); сложность не настраивается и всегда соответствует 90 % точности ответов ботов.
- В матче участвуют две команды: пара живых игроков и пара ботов «Бот Атлас» и «Бот Глобус». Очередность ходов циклическая и всегда одна и та же: Игрок 1 → Бот Атлас → Игрок 2 → Бот Глобус → снова Игрок 1 и так далее.
- Каждая карточка формируется со случайным направлением (страна→столица или наоборот) и рассылается всем участникам: активный человек получает сообщение с инлайн-клавиатурой, второй игрок и оба бота — тот же текст без кнопок (ботам вопрос сохраняется для логирования, но сообщение не удаляется).
- Пока пара не решена верно, вопрос переходит к следующему участнику в очереди. Верный ответ удаляет пару из очереди; при ошибке ход переходит дальше без смены карточки.
- Два бота отвечают автоматически с фиксированной точностью около 90 %. Их сообщения о неверных попытках и успехах приходят в лички игроков так же, как ответы напарника.
- После каждого ответа бот-организатор отправляет обоим игрокам актуальный счёт: суммарные очки команды людей и общий результат команды ботов с названиями «Бот Атлас» и «Бот Глобус».
- При верном ответе любого участника рассылается сообщение с флагом и текстом «<имя> отвечает верно! Правильный ответ: <страна> — <столица>» плюс кнопка «Еще один факт».
- Пауза между вопросами составляет около 2 секунд (допустимый диапазон 2–3 секунды).
- Матч завершается, когда все пары использованы. Финальное сообщение содержит строку «Игра завершена», итог команды игроков (с перечислением имён) и общий счёт команды ботов («Бот Атлас и Бот Глобус»), после чего выводится победитель: команда игроков, команда ботов или ничья.
...
- предоставляет пять режимов: **Флэш-карточки**, **Игра на время**, **Кооператив**, **Учить по списку стран**, **Тест**;
...
2. После регистрации обоих участников бот предлагает выбрать континент (или «Весь мир (30 пар)»); направление вопросов всегда смешанное. Сложность не настраивается: оба бота отвечают правильно примерно в 90 % случаев.
...
- Администратор выбирает континент, после чего матч сразу стартует с фиксированной точностью ботов (≈90 %).
