...
Сделать Telegram-бота с вебхуком (**FastAPI**), реализующего четыре режима обучения/тренировки по соответствиям **страна ↔ столица**.

---

## ПРАВИЛА И РЕЖИМЫ

Бот помогает выучить и натренировать соответствия **страна ↔ столица** четырьмя комплементарными режимами. Во всех режимах пользователь предварительно может выбрать **континент** (Европа / Азия / Африка / Северная Америка / Южная Америка / Австралия и Океания / Весь мир) и **тип вопросов**:
- *Страна → столица*;
...
- После выбора варианта бот показывает тост: «✅ Верно» или «❌ Неверно. Правильный ответ: …» (для длинных сообщений используется `show_alert=True`).
 - При верном ответе сообщение заменяет карточку, а бот дополнительно отправляет случайный факт (≤150 символов) о стране/столице (см. раздел «LLM‑кэш и обновление»).
...
- предоставляет четыре режима: **Флэш-карточки**, **Игра на время**, **Дуэт против Бота**, **Учить по списку стран**;
...
│   ├── facts.py                # случайные факты (см. «LLM‑кэш и обновление»)
...
- `FACTS_CACHE_PATH` — путь к JSON-кэшу фактов (см. «LLM‑кэш и обновление»)
 
## 🤖 LLM‑кэш и обновление

Случайные факты о странах и столицах сохраняются в кэше в формате **JSON**. Путь к файлу задаётся переменной окружения `FACTS_CACHE_PATH`. Фоновая задача проверяет возраст кэша и не реже одного раза в **5 дней** обновляет данные, обращаясь к LLM и перезаписывая файл. Обновление выполняется в фоне и не блокирует работу бота.
