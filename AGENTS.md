...
- Инициация матча идёт через личку: первый игрок отправляет боту **/coop_capitals** и получает код приглашения. Второй игрок присоединяется командой **/coop_join <код>**.
...
- Настройки: число раундов (5/10/15), сложность бота-соперника:
  ...
  - 🟡 Средний ≈ 60%;
  - 🔴 Сложный ≈ 80%.
- Каждый раунд: один и тот же вопрос на двоих. Оба игрока отвечают в своих личках (кнопки 4 варианта). Верный ответ игрока → **+1 очко Команде**.
  Затем отвечает «бот-соперник» (верно/неверно по вероятности сложности) → **+1 очко Боту** при верном ответе.
- После ответов обоих живых игроков бот формирует итог раунда и отправляет его в обе лички. При верном ответе добавляется название страны, строка «Столица: {capital}» (если вопрос про столицу) и изображение флага.
