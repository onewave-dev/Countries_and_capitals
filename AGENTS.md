...
Команды **/quit** и **/exit** полностью завершают текущую сессию для всех участников и выводят сообщение «Сессия прекращена. Для запуска прогаммы используйте /старт».
...
- Инициация матча идёт через личку: первый игрок отправляет боту **/coop_capitals**, вводит имя и получает кнопку‑приглашение с кодом. Второй игрок вводит имя и присоединяется по кнопке или командой **/coop_join <код>**.
- Общение идёт только через DM. Бот после каждого раунда рассылает итог обоим игрокам, выполняя роль «виртуального общего чата».
- Направление вопросов всегда **смешанное** и не настраивается.
- Настройки: число раундов (5/10/15), сложность бота-соперника:
  - 🟢 Лёгкий ≈ 30% верных ответов;
  - 🟡 Средний ≈ 60%;
  - 🔴 Сложный ≈ 80%.
- Каждый раунд: один и тот же вопрос на двоих.
- Игроки отвечают по очереди в своих личках (кнопки 4 варианта): сначала Игрок 1, затем Игрок 2. Верный ответ → **+1 очко Команде**.
- После ответов игроков бот делает короткую паузу и отвечает как соперник (верно/неверно по вероятности сложности) → **+1 очко Боту** при верном ответе.
- Бот формирует итог раунда и отправляет его в обе лички. При верном ответе добавляется название страны, строка «Столица: {capital}» (если вопрос про столицу) и изображение флага.
...
- После каждого ответа отображается карточка с флагом.
...
- Далее через пустую строку идут название страны и, если вопрос был «страна → столица», строка «Столица: {capital}».
...
### Дуэт против Бота
1. Игрок 1 в личке отправляет **/coop_capitals**, вводит имя и получает кнопку‑приглашение с кодом. Игрок 2 вводит имя и подключается по кнопке или командой **/coop_join <код>**.
2. После регистрации обоих участников бот предлагает выбрать число раундов и сложность соперника.
3. Каждый раунд: бот задаёт вопрос → отвечает Игрок 1 → отвечает Игрок 2 → после короткой паузы отвечает бот‑соперник.
4. Начисление очков, инкремент раунда, пауза 2–3 сек и рассылка итогов обоим игрокам.
5. Завершение после выбранного числа раундов → объявление результата и кнопка «🔄 Сыграть снова».
...
- Сервер предоставляет корневой маршрут `/` (или `/healthz`), отвечающий **200** для health-check и поддерживающий `HEAD /` для проверок балансировщика.
