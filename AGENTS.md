...
- Время на ход не ограничено; между вопросами соблюдаем паузу из диапазона 2–3 с (сейчас фиксированная задержка `asyncio.sleep(2)` в `_next_turn`, событие логируется DEBUG-сообщением «Coop next question scheduled after …»).
...
- Включить DEBUG-логи по ключевым точкам:
  ...
  - начисление очков,
  - срабатывание пауз между вопросами (DEBUG: «Coop next question scheduled after …»),
  - финальные итоги.
