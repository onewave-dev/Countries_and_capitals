...
Бот помогает выучить и натренировать соответствия **страна ↔ столица** пятью комплементарными режимами. Во всех режимах пользователь предварительно может выбрать **континент** (Европа / Азия / Африка / Северная Америка / Южная Америка / Австралия и Океания / Весь мир) и **тип вопросов**:
- *Страна → столица*;
...
Для администратора в меню доступен отдельный пункт «[адм.] Тестовая игра» для быстрой проверки кооперативного режима.
...
### 3) «Кооператив: столицы мира» (2 игрока)
...
- Перед стартом выбирается континент (Европа / Азия / Африка / Северная Америка / Южная Америка / Австралия и Океания / Весь мир (30 пар)).
- Направление каждого вопроса определяется случайно.
- Игроки отвечают по очереди в своих личках (кнопки 4 варианта): сначала Игрок 1, затем Игрок 2. Верный ответ удаляет пару из очереди. При ошибке бот сообщает правильный ответ и возвращает пару в конец очереди.
- После каждого вопроса бот рассылает итог обоим игрокам, выполняя роль «виртуального общего чата».
- Время на ход не ограничено; между вопросами пауза 2–3 секунды.
- Матч завершается, когда все пары исчерпаны. Бот отправляет итог и список ошибочных пар (если были).
...
- Кооперативные матчи — в `chat_data['sessions'][session_id]`:
  - участники, параметры матча (континент, тип вопросов), очередь пар, чья очередь, список неверных ответов, активные джобы пауз.
...
- Для кооператива — один и тот же вопрос показывается обоим игрокам; при неверном ответе пара возвращается в конец очереди.
...
2. После регистрации обоих участников бот предлагает выбрать континент (или «Весь мир (30 пар)»); направление вопросов всегда смешанное.
3. Бот задаёт вопрос → отвечает Игрок 1 → отвечает Игрок 2. При неверном ответе пара возвращается в конец очереди.
4. После каждого вопроса бот рассылает итог обоим игрокам и делает паузу 2–3 сек для читаемости.
5. Завершение после исчерпания всех пар → итог и список ошибок (если были), кнопка «🔄 Сыграть снова».
